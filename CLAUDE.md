...
1. **解锁后要 `scrollTo` 回去。** body 固定期间文档滚动量是 0，
   不还原的话一关抽屉就弹到页首。
1b. **而且那一下必须是瞬间的。** `html` 上有 `scroll-behavior: smooth`
   （styles/base.css:13），于是 `scrollTo` 变成一段动画 —— 实测在 2055 关掉抽屉，
   scrollY 先掉到 **0**，再花约 1.5 秒滑回 2054.5。所以还原前把
   `documentElement.style.scrollBehavior` 临时改成 `auto`，还原完把**原来的内联值**
   放回去（原来多半是空串，直接写 `"auto"` 会永久盖掉那条全局规则）。
   同一个理由，焦点还给汉堡时要用 **`focus({ preventScroll: true })`** ——
   不然浏览器为了「把汉堡露出来」再滚一次，刚跳回去的位置又被滑走。
...
- `lock.mjs`（4 个宽度 × 9 条）—— 锁上之后滚轮和 `scrollTo` 都推不动、
  抽屉自己仍然能滚、Esc 关掉回到原来的位置、点导航走掉新页面从顶上开始。
  **两头都验过**：撤掉修复重跑，背景从 374 被推到 1115，12 条失败。
- `lock2.mjs`（360 / 390 / 768 / 960 × 三种关法 × 16 条）—— 专测「还原是不是瞬间的」。
  **它必须逐帧采样，不许等平滑滚动跑完再量**：在页面上下文里触发关闭，
  然后连采五个 `requestAnimationFrame`，断言第二帧就到位、第一帧没掉到 0、
  帧与帧之间不再变化。它先断言 `getComputedStyle(html).scrollBehavior === "smooth"`，
  否则这个测试没有意义。撤掉修复重跑，逐帧是 `0 → 0 → 1 → 4 → 9`，
  900ms 之后还差 1000px，48 条失败。
