...
function hrefWith(q: CodingQuery, patch: CodingQuery) {
  const next = { ...q, ...patch };
        ...
        <div className="filters">
          ...
          {TRACKS.map((t) => (
            <Link
              ...
              data-on={track === t.id}
              href={hrefWith(query, { track: t.id })}
              key={t.id}
            >
              <T en={t.en} zh={t.zh} />
            </Link>
            ...
            {TRACKS.filter((t) => t.id !== "all").map((t) => (
              <div key={t.id}>
                ...
                <span className="dimmer tabular" style={{ float: "right" }}>
                  {all.filter((p) => p.track === t.id).length}
                </span>
              </div>
            ))}
          </div>
