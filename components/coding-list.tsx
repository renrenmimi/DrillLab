...
export function CodingList({ query }: { query: CodingQuery }) {
  ...
  return (
    ...
    // 原来右栏那两块（进度、为什么有的跑不了）一块变成页头下面那条 .ui-prog，
    // 一块落到页尾的安静分区里。
        ...
        <div className="ui-page mode-page">
          ...
          <div className="ui-sec">
            ...
            {shown.length === 0 ? (
              ...
              // 每张卡是 .ui-card[data-rows]：题名 / 标签 / 页脚三层，
              // 标签那一层占 1fr。所以四个标签换行时，页脚上那枚
              // 「已完成 / 未开始」仍然和邻卡对在同一条底线上。
              //
              // 整张卡就是那个链接（和老版本「整行左半边都是链接」同一个意思）——
              // 里面没有第二个可点的东西，徽章是纯文字。
              <ol className="ui-cards cd-list">
                {shown.map((p) => (
                  <li key={p.id}>
                    <Link className="ui-card cd-card" data-rows href={codingPath(p.id)}>
                      ...
                      <span className="cd-meta">
                        ...
                        <span
                          className="tag"
                          data-tone={runnableInBrowser(p) ? "accent" : undefined}
                        >
                          {runnableInBrowser(p) ? (
          ...
          {/* 【右栏没了】四页统一 data-rail="off"，所以这一块落到页尾这个
              明显低一档的分区里。
              「按方向」那一块早就搬走了 —— Practice 模式的侧栏里就是那份清单，
              而且那份带筛选状态。同一份东西不摆两遍。 */}
